...
FWD_ST            BRSET   PORTAD0, $04, NO_FWD_BUMP           ; Checks if bow bumper is hit                           
                  ...
                  ; --- MODIFIED: do a short reverse and let the REV_TRN state handler
                  ;     perform the turn. Removing immediate INIT_RIGHT and long delays
                  JSR     INIT_REV
                  LDY     #1200               ; small reverse duration (tunable)
                  JSR     del_50us
                  LBRA    EXIT                                                                     ; return to main loop and handle rev turn in REV_TRN_ST
...
REV_TRN_ST        ; --- MODIFIED: perform a short reverse, then initiate a LEFT_TRN and let LEFT_TRN handler manage the turn
                  JSR     INIT_REV                ; back up a little first
                  LDY     #1200
                  JSR     del_50us
                  ; start a left turn (INIT_LEFT sets T_TURN relative to TOF_COUNTER)
                  JSR     INIT_LEFT
                  MOVB    #LEFT_TRN, CRNT_STATE
                  ; do NOT immediately set FWD or INIT_FWD here — let LEFT_TRN complete the turn
                  BRA     EXIT                                                                     ; return to main loop
...
; [the rest of the code is unchanged — READ_SENSORS, LCD, display, utils, TOF ISR, etc.]
